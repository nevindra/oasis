...
| **Output schema**        | `outputSchema` validated; can declare `toModelOutput` transform and `transform` policy  | None — output is just `json.Marshal`'d; no validation                                | Mastra |
| **Input validation**     | **6-step coercion pipeline** (`tools/validation.ts:444`) — silently handles LLM quirks: null→{}, stringified JSON args (GLM4.7), null-for-optional (Gemini), `query`/`message`/`input`→`prompt` (Claude Sonnet drift). Pipeline IS the fallback strategy; no strict/warn/fallback mode enum | Single-pass `json.Unmarshal`; missing fields get Go zero values; no coercion, no retries | Mastra |
...
| **Per-tool observability** | `TOOL_CALL` / `MCP_TOOL_CALL` span per tool execution with input/output/success attributes | Span at agent-execution level only; per-tool data captured in `StepTrace`, not OTel spans | Mastra |
...
| **Per-tool retry / timeout** | Via `ToolBackgroundConfig.timeoutMs` and `maxRetries`                                | None at tool definition level; MCP tools default to 60s (`mcp/tool_wrapper.go:12`)   | Mastra |
...
**Score: Mastra 9 — Oasis 4 — Tie 6 — N/A 1**
...
| **Semantic Recall**             | **Dedicated external vector DB** (Pinecone/pgvector/Qdrant/Chroma/Lance/+10) with native ANN; index name pattern `memory_messages_${model}_${dimension}`; resource- or thread-scoped vector filter | **Inline `embedding TEXT` column** in messages table; **brute-force O(N) cosine** scan in process; `CrossThreadSearch(embedder)` with `chatID` filtered after SQL returns | Mastra |
...
| **Event type count**          | **~70 distinct chunk types** across `AgentChunkType` (~35), `NetworkChunkType` (~20 network-specific), `WorkflowStreamEvent` (~11)                | **16 event types** in a single `StreamEvent` struct                                         | Mastra |
| **Reasoning chunks**          | First-class: `reasoning-start` / `reasoning-delta` / `reasoning-end` / `reasoning-signature` (cryptographic) / `redacted-reasoning` | Single `EventThinking` event with content                                                   | Mastra |
...
| **Structured object streaming** | `objectStream: ReadableStream<Partial<OUTPUT>>` + `elementStream` for array outputs               | N/A                                                                                         | Mastra |
...
| **Output convenience accessors** | `MastraModelOutput` exposes ~25 promise-based accessors: `.text`, `.reasoning`, `.reasoningText`, `.sources`, `.files`, `.steps`, `.toolCalls`, `.toolResults`, `.usage`, `.totalUsage`, `.warnings`, `.finishReason`, `.providerMetadata`, `.object`, `.suspendPayload`, `.resumeSchema`, `.textStream`, `.objectStream`, `.elementStream`, `.tripwire`, `.error`, `.getFullOutput()` — all lazy | `AgentResult{Output, Usage{Input/Output/CachedTokens}, Steps []StepTrace, Attachments}` synchronously available when `ExecuteStream` returns | Mastra |
| **Multi-reader / fan-out**    | `#bufferedChunks` replay buffer: every new `fullStream` reader gets **full chunk history** from the start (broadcast/replay, not queue) | Single-reader channel — fan-out requires manual forwarding                                  | Mastra |
...
| **Per-stream observability**  | Provider span (`llm: '<modelId>'`, `SpanType.MODEL_GENERATION`) per LLM call with model/provider/temp/maxTokens attrs; ended in `onFinish` with `finishReason` + `usage` | Single `agent.execute` span wraps the whole stream; no per-iteration / per-tool spans in core; `StepTrace` records carry per-tool timing | Mastra |

**Score: Mastra 11 — Oasis 3 — Tie 2**

> The previous version of this doc said Mastra had "10+ chunk types" — the actual count is **~70 across the three union types** (AgentChunkType / NetworkChunkType / WorkflowStreamEvent). Background-task events alone account for 10 chunk types. Oasis has **16 event types**, not the 5 I had listed. The `streamLegacy()` distinction is on AI SDK `specificationVersion: 'v1'`, not literally "v4 vs v5".
>
> Architecturally: Mastra streaming is *promise-rich* — you can `await stream.text` without iterating; new readers get full chunk history replayed; tool args stream in fragments. Oasis streaming is *channel-flat* — single reader, single struct type, but the producer naturally pauses when the consumer slows.
...
| **Framework-initiated suspend**      | `requireApproval: true` on tool def → framework pauses before execution; emits `tool-call-approval` stream chunk (`tool-call.ts:222`) | None — no framework-enforced "this tool always needs approval" gate                    | Mastra |
| **Approval API**                     | First-class agent methods: `agent.approveToolCall({runId})` / `declineToolCall()` / `approveToolCallGenerate()` / `declineToolCallGenerate()` (`agent.ts:6835–6861`) | Through processor pattern + `InputHandler.RequestInput` block                          | Mastra |
...
- **Zero-overhead observability** — `Tracer`/`Span` interfaces in root package with no OTEL imports; `StepTrace` on every `AgentResult`
