...
| **Step types**                | Plain, parallel, conditional, foreach, dowhile, dountil, sleep, sleepUntil; agent/tool via `createStep(agent)` | `Step`, `AgentStep`, `ForEach`, `DoWhile`, `DoUntil` as first-class options; single-tool wrappers via `AgentStep` + `agent.WithTools(...)` | Tie |
...
| Feature                         | Mastra                                                                                              | Oasis                                                                                  | Winner |
|---------------------------------|-----------------------------------------------------------------------------------------------------|----------------------------------------------------------------------------------------|--------|
| **Architecture**                | 3 named processors (`MessageHistory`, `WorkingMemory`, `SemanticRecall`) + `ObservationalMemory` engine (replaces `MessageHistory` rather than composing); `Memory` class 2,873 LOC + ObservationalMemory 3,633 LOC | **Unified `MemoryItem` discriminated by `Kind` (fact/note/event/playbook/reflection/summary; user-extensible)** + two composable processor pipelines (ingest write-path, retrieve read-path) + single `oasis.WithMemory(...)` entry point. One record type, one `ItemStore` interface, one `Filter`. `WithUserMemory` and `WithHistory` removed (pre-v1 hard cut). | Tie |
| **Message History**             | `MessageHistory` processor; `lastMessages` window with ID-based dedup against existing messages     | `LoadHistory` retrieve processor → `core.Store.GetMessages(threadID, limit)`; `memory.WithMaxHistory(n)` + `memory.WithMaxTokens(n)` (`memory/retrieve_processors.go`) | Tie    |
| **Working Memory**              | LLM-writable scratchpad — Markdown template or Zod schema; resource- or thread-scoped; mutex-protected writes via `async-mutex` keyed `resource-${id}` / `thread-${id}`; LLM updates via `updateWorkingMemory` tool | `memory.WithWorkingMemory()` enables a Markdown scratchpad as a `KindNote` `MemoryItem` with deterministic ID per `(agentName, scope)` via `memory.WorkingMemoryID(...)` so `Remember` overwrites the same row; resource-scoped by default, `memory.WithWorkingMemoryScope(s)` overrides (Thread/Resource/Agent); LLM writes via the opt-in `memory.remember` tool. **Markdown only in v1** — no schema validation. | Mastra |
...
| **Semantic Recall**             | **Dedicated external vector DB** (Pinecone/pgvector/Qdrant/Chroma/Lance/+10) with native ANN; index name pattern `memory_messages_${model}_${dimension}`; resource- or thread-scoped vector filter | **Inline `embedding` JSON column** in `memory_items` (and `messages`) tables; **brute-force O(N) cosine** scan in process. `BatchedRecall` retrieve processor does **one** `SearchSemantic(emb, Filter{Kinds, Scope}, topK)` call across all configured kinds — splits results into per-kind prompt slots — replacing the previous three separate fact/event/note recall calls. `RecallCrossThread` keeps the cross-thread messages query as a separate roundtrip. **Two DB roundtrips per turn** total. | Mastra |
| **Semantic Trimming**           | Via `SemanticRecall` topK + range window                                                            | `memory.WithSemanticTrimming()` with `memory.WithKeepRecent(n)` anchor (default 3); separate trim embedding via `memory.WithSemanticTrimEmbedding(e)` so a smaller/faster model can be used; `embeddingCache` in-memory hot path for repeated trim queries | Tie |
| **Token-budget Trimming**       | `lastMessages` count window only; no token-budget trim in core processors                           | `memory.WithMaxTokens(n)` + oldest-first fallback; estimate `runeCount/4 + 4`          | Oasis  |
...
| **Per-turn compression**        | N/A (handled by Observational Memory)                                                               | `memory.WithCompress(fn, threshold)` — LLM-driven in-flight summarization without persistence dependency. Trigger lives framework-side in the agent loop; policy supplied by app via `core.ModelFunc`. | Oasis |
| **User Memory (Facts)**         | Subsumed by Working / Observational Memory                                                          | First-class as `KindFact` items in the unified MemoryItem. LLM extraction via `FactExtractor` ingest processor (provenance: `Source{Kind:"extraction", Ref:<message-id>, AgentID}`); semantic dedup ≥0.85 + supersession via `Deduper`; probabilistic decay ~5%/turn via `DecayProbabilistic` (deletes unpinned facts older than 30 days). Same code path used by the `memory.remember`/`memory.recall` LLM tools and the `agent.Memory().Remember/Recall/Forget` developer API. | Oasis |
| **Memory kinds (extensibility)**| Implicit — message/observation/working memory hardcoded as separate engines | `Kind` is an **open string type** with 6 canonical built-ins (`KindFact`, `KindNote`, `KindEvent`, `KindPlaybook`, `KindReflection`, `KindSummary`); apps can introduce `"decision"`, `"hypothesis"`, or any custom kind in app code and every pipeline + tool + filter operates on them identically without framework changes. | Oasis |
| **Scope**                       | `threadId` + `resourceId` + cross-resource (admin)                                                  | `Scope{Kind, Ref}` with 4 visibility partitions: `ScopeThread` / `ScopeResource` / `ScopeAgent` / `ScopeGlobal`. v1 pipelines consume Thread + Resource; Agent/Global slots reserved for future cross-user/cross-agent memory. Scope is filterable via `Filter.Scope`. | Tie |
| **Provenance**                  | Implicit (DB row tracks source)                                                                     | `Source{Kind, Ref, AgentID}` first-class on every `MemoryItem`; records origin (`extraction`/`tool`/`user`/`agent`/`message`) with a foreign key back to the producing message or tool call. Enables "where did I learn this?" queries and source-filtered forgetting. | Oasis |
| **Pinned items**                | Not first-class                                                                                     | `Pinned bool` on every `MemoryItem`; `LoadPinned` retrieve processor always injects pinned items into the prompt (independent of TopK / score); `memory.pin` tool toggles the flag; `Filter.Pinned *bool` for include/exclude in queries; pinned items are **never auto-decayed**. | Oasis |
| **Agent-callable memory tools** | `updateWorkingMemory` tool (working memory only)                                                    | `memory.WithTools(memory.AllTools()...)` registers four tools — `memory.remember` / `memory.recall` / `memory.forget` / `memory.pin` — letting the LLM autonomously save, search, delete, and pin any MemoryItem kind. Default **OFF**; opt-in. | Tie |
| **Composable pipelines**        | 3 named processors with hardcoded order; `ObservationalMemory` replaces `MessageHistory` rather than composing                          | `IngestProcessor` (write path) and `RetrieveProcessor` (read path) interfaces with **9 + 7 built-in processors** (EnsureThread, PersistMessages, FactExtractor, Deduper, Embedder, Upserter, TitleGenerator, DecayProbabilistic, EventRecorder; EmbedInput, LoadHistory, LoadPinned, BatchedRecall, RecallCrossThread, TrimToBudget). `memory.WithIngestProcessors(...)` / `memory.WithRetrieveProcessors(...)` append user-supplied processors after defaults — full replacement deferred to v2. | Oasis |
| **Developer API surface**       | `agent.memory.recall(...)` / `.save(...)` / `.update(...)` / `.delete(...)` + Working Memory mutex API | `agent.Memory()` returns `*memory.AgentMemory` with `Remember(ctx, item) error`, `Recall(ctx, query, opts...) ([]ScoredItem, error)`, `Forget(ctx, ForgetSpec) (int, error)`, `List(ctx, Filter) ([]MemoryItem, error)`, `Get(ctx, id) (MemoryItem, error)`, `Pin(ctx, id, pinned bool) error`. Same code path that the agent-callable tools delegate through. | Tie |
| **Compaction**                  | Via Observational Memory reflection only                                                            | Three mechanisms, all opt-in via `memory.Option`: inline tool-result truncation; per-turn `memory.WithCompress(fn, threshold)`; persistent threshold-based `memory.WithCompaction(c, threshold)` with `core.Compactor` interface (app supplies policy, framework owns trigger). Stock `StructuredCompactor` (9-section format) ships in `compaction/` satellite. | Oasis |
| **Injection Guard at memory**   | N/A                                                                                                 | `sanitizeRawFacts()` + `containsInjectionPattern()` — **11-entry** blocklist (`[system`, `[assistant`, `<|im_start|>`, `ignore previous`, etc.) + LLM-prompt-level guard in `extractFactsPrompt`; deliberately narrow to avoid false positives (`memory/ingest_processors.go`) | Oasis |
| **Persist Backpressure**        | `SaveQueueManager` debounces async persistence (100ms window, immediate flush at 1s stale)          | Bounded semaphore `maxIngestGoroutines=16` + 30-second wait + drop or "lightweight persist" fallback (skip extraction/embedding, still write messages); `context.WithoutCancel` detaches from request; `AgentMemory.Close()` waits on `sync.WaitGroup`. | Oasis |
...
| **Memory pagination**           | Full pagination in `recall()`: `perPage`, `page`, `orderBy`, date-range filtering, context-window `include` around target messages | `Filter{Limit, Since, Until}` supports time-window + size cap on `ItemStore.List`; no offset/cursor for messages (`GetMessages(threadID, limit)` is most-recent N only) | Mastra |
...
| **Runtime per-call override**   | `RequestContext.set('MastraMemory', { memoryConfig: { lastMessages: 5 } })` for per-request overrides | Via `RunOptions.Memory` field passed through `ExecuteWith(ctx, task, &RunOptions{Memory: alt})` — full memory swap, not partial field override | Tie |
| **Embedding dimension handling**| Auto-detected once and cached; dimension-aware index names                                          | Caller chooses embedding provider; brute-force scan tolerant to dimension; no index-name partitioning | Mastra |
...
| **Schema migrations**           | `CREATE TABLE IF NOT EXISTS` per adapter; no in-code migration framework                            | Idempotent `CREATE TABLE IF NOT EXISTS memory_items` per backend; no version tracking; **hard cut from old `user_facts` table — no auto-migration of pre-v0.17 data** | Tie    |
| **Observability**               | OTel spans per memory op (`recall`/`save`/`update`/`delete`); REST `GET /memory/status` exposes OM buffer state | `agent.memory.load` / `agent.memory.ingest` spans via `core.Tracer` (attribute-rich: `thread_id`, processor names); structured `slog.Logger`; no REST inspection | Mastra |

**Score: Mastra 12 — Oasis 10 — Tie 8**

> **2026-05-23 (post memory redesign):** Memory category overhauled in [`docs/superpowers/specs/2026-05-23-memory-redesign-design.md`](../superpowers/specs/2026-05-23-memory-redesign-design.md). Pre-v1 hard cut: `WithUserMemory` and `WithHistory` deleted, `oasis.WithMemory(...)` is the single entry point. Net category shift: **Mastra 13 / Oasis 7 / Tie 4 → Mastra 12 / Oasis 10 / Tie 8**.
>
> Rows that flipped Mastra → Tie:
> - **Working Memory**: Oasis now ships `memory.WithWorkingMemory()` + `KindNote` + `WorkingMemoryID(agent, scope)` stable-ID helper. Markdown only in v1 (Mastra still leads on Zod-schema-validated working memory).
> - **Runtime per-call override**: `RunOptions.Memory` already exists; describing it accurately here.
> - **Schema migrations**: equally minimal on both sides now.
>
> Rows that flipped Mastra → Oasis:
> - **User Memory (Facts)**: was already Oasis; description rewritten — facts are now `KindFact` items in the unified MemoryItem, extracted by the `FactExtractor` ingest processor with first-class provenance via `Source{Kind, Ref, AgentID}`.
>
> New rows that lean Oasis or Tie:
> - **Memory kinds (extensibility)** (Oasis): `Kind` is an open string type, not a closed enum; apps add custom kinds and every pipeline + tool + filter works on them identically.
> - **Scope** (Tie): 4 visibility partitions (Thread/Resource/Agent/Global) on `MemoryItem` vs Mastra's 3 (thread/resource/cross-resource).
> - **Provenance** (Oasis): `Source{Kind, Ref, AgentID}` first-class on every record.
> - **Pinned items** (Oasis): `Pinned bool` + always-included `LoadPinned` retrieve processor + `memory.pin` LLM tool + never-auto-decayed semantics.
> - **Agent-callable memory tools** (Tie): four opt-in tools (`memory.remember` / `memory.recall` / `memory.forget` / `memory.pin`) cover the full LLM-driven memory loop, vs Mastra's working-memory-only `updateWorkingMemory`.
> - **Composable pipelines** (Oasis): 9 ingest + 7 retrieve built-in processors, all individually swappable, with `WithIngestProcessors`/`WithRetrieveProcessors` append hooks.
> - **Developer API surface** (Tie): `agent.Memory()` returns a struct with `Remember/Recall/Forget/List/Get/Pin`; same code path as the LLM tools.
>
> What Mastra still leads on:
> - **Observational Memory** — 3,600-line observe→reflect engine remains unmatched.
> - **Semantic Recall via external ANN-indexed vector DB** — sub-linear retrieval vs Oasis's brute-force O(N) cosine scan over inline embeddings. The single largest scale-related gap in this category.
> - **Token counter sophistication, thread cloning, multi-modal persistence, REST API, RBAC/FGA on memory, embedding dimension handling, observability surface** — all unchanged.
>
> Architectural contrast: Mastra's memory is **LLM-assisted at retrieval time** (Working Memory tool, Observational Memory reflection); Oasis's is **pre-processed before the LLM call** (facts pre-extracted, recall batched into one roundtrip, pinned items always loaded) — cheaper per turn, more deterministic, less adaptive. The redesign closes the working-memory-as-feature gap and unifies the API surface, but the dedicated-vector-DB and observational-reflection gaps are unaddressed and remain real.
...
> - **2026-05-23 (post memory redesign)**: Mastra 88 / Oasis 73 / Tie 64 — Memory category shifted Mastra 13/7/4 → Mastra 12/10/8 via [`docs/superpowers/specs/2026-05-23-memory-redesign-design.md`](../superpowers/specs/2026-05-23-memory-redesign-design.md). Pre-v1 hard cut: `WithUserMemory` and `WithHistory` deleted, `oasis.WithMemory(...)` is the single entry point. New unified `MemoryItem` discriminated by `Kind` (fact/note/event/playbook/reflection/summary; user-extensible string type) replaces the old facts+history split. Two composable processor pipelines (9 ingest, 7 retrieve built-in processors) drive write and read paths. `BatchedRecall` collapses three previous separate fact/event/note queries into one DB roundtrip. Working memory enters Oasis as a Markdown-only `KindNote` item with stable ID per `(agentName, scope)` (Mastra still leads on Zod-schema-validated working memory). Four opt-in agent-callable tools (`memory.remember` / `memory.recall` / `memory.forget` / `memory.pin`) cover the full LLM-driven memory loop. Compaction trigger stays framework-level via `memory.WithCompaction(c, threshold)`; policy in `core.Compactor` interface (app-supplied). New first-class concepts: `Scope{Kind, Ref}` (4 visibility partitions), `Source{Kind, Ref, AgentID}` provenance on every record, `Pinned bool` (always-included via `LoadPinned` processor, never auto-decayed). Mastra still leads on Observational Memory (3.6k-line observe→reflect engine), dedicated-ANN-vector-DB-backed semantic recall, token counter sophistication, thread cloning, multi-modal persistence, REST API, memory RBAC/FGA. Architectural posture unchanged: Oasis memory is pre-processed before the LLM call (cheaper per turn, more deterministic); Mastra is LLM-assisted at retrieval time (more adaptive).
...
- **Unified `MemoryItem` discriminated by user-extensible `Kind`** — one record type covers facts, working memory, events, playbooks, reflections, summaries; apps define new kinds (`"decision"`, `"hypothesis"`, …) without framework changes
- **Composable ingest + retrieve processor pipelines** — 9 + 7 built-in processors, each individually swappable; `memory.WithIngestProcessors(...)` / `WithRetrieveProcessors(...)` append user-supplied steps; `BatchedRecall` collapses N kind-specific queries into one DB roundtrip
- **First-class `Source{Kind, Ref, AgentID}` provenance** on every `MemoryItem` — enables "where did I learn this?" queries and source-filtered forgetting
- **First-class `Scope{Kind, Ref}` visibility partition** — Thread/Resource/Agent/Global with the framework consuming Thread + Resource in v1 (Agent + Global reserved for cross-user/cross-agent memory)
- **First-class `Pinned bool` memory** — `LoadPinned` retrieve processor always injects pinned items into the prompt; `memory.pin` LLM tool toggles the flag; pinned items are never auto-decayed
- **Four opt-in agent-callable memory tools** — `memory.remember` / `memory.recall` / `memory.forget` / `memory.pin` cover the full LLM-driven memory loop; default OFF (`memory.WithTools(memory.AllTools()...)` to enable)
- **User memory fact store** — `FactExtractor` ingest processor + LLM extraction, semantic dedup ≥0.85 (`Deduper`), supersession via tag mechanism, probabilistic decay 5%/turn (`DecayProbabilistic`)
- **Injection guard at memory layer** — `sanitizeRawFacts()` + `containsInjectionPattern()` blocklist (11 phrases, deliberately narrow to avoid false positives) (`memory/ingest_processors.go`)
- **Persist backpressure** — bounded ingest semaphore (`maxIngestGoroutines=16`), lightweight fallback (skip extraction/embedding when saturated), 30-second drop deadline, `AgentMemory.Close()` waits on `sync.WaitGroup`
...
2. **Working memory schema validation** — Markdown-only working memory shipped in 2026-05-23; a Zod-equivalent struct-schema-validated variant (LLM writes typed JSON instead of free-form Markdown) would close the remaining gap with Mastra's Working Memory.
...
>
> ✅ **Partially closed in 2026-05-23 (post memory redesign):** Working memory primitive (gap #2 above) now ships as a Markdown-only `KindNote` item with deterministic ID per `(agentName, scope)` via `memory.WithWorkingMemory()` + `memory.WorkingMemoryID(...)`. Remaining gap: schema-validated typed working memory (Zod/Go-struct equivalent). Also unified the memory API surface — `WithUserMemory` and `WithHistory` collapsed into one `oasis.WithMemory(...)` entry point with composable ingest + retrieve processor pipelines and four opt-in agent-callable tools.
