...
**Per-thread compaction** (`Compactor`) summarizes the entire conversation into a 9-section structured document that is persisted back into the thread. Subsequent turns load the compact plus any new exchanges — the model sees a stable, dense context that survives across calls. This is the right strategy for long-lived assistants.
...
The threshold (`0.80`) is a fraction of the effective context window. When estimated tokens cross threshold × window, the Compactor runs against the loaded history. Passing `nil` as the Compactor or `threshold ≤ 0` disables the option (noop).
...
`WithCompaction` is a convenience. It fires the Compactor automatically when history crosses the threshold, during the conversation-memory load path.

Consumers that orchestrate compaction themselves — deciding when to compact, handling re-compact ordering, merging compacts from multiple threads, or persisting compacts into application-specific state — should NOT pass `WithCompaction`. They construct pre-compacted message lists at the application layer and hand the agent a ready-to-run history.
