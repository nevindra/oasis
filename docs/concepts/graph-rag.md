...
The `Description` field stores a human-readable explanation of why the relationship exists, as generated by the LLM during extraction. This description is surfaced in retrieval results via `GraphContext`. Sequence edges (created by `WithSequenceEdges`) have an empty description — they are deterministic, not LLM-generated.
...
| `WithLLMTimeout(d)` | 2 min | Maximum duration for individual LLM calls during graph extraction and contextual enrichment |
...
Both shipped backends implement all required interfaces.
...
- [Retrieval](rag.md) — `GraphRetriever` and `HybridRetriever` details
