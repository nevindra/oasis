...
1. **Batch formation** — chunks are grouped into batches of `graphBatchSize` (default 5). Each batch must contain at least 2 chunks.
2. **Prompt construction** — each batch is formatted into a structured prompt with chunk IDs and content:
   ...
   ```
3. **LLM call** — the batch is sent to the configured `Provider` via `Chat()`. The prompt instructs the LLM to output JSON edges with source, target, relation type, and confidence weight.
...
5. **Context cancellation** — the batch loop checks `ctx.Err()` before each iteration, stopping early if the context is cancelled.
6. **Graceful degradation** — individual batch failures (LLM errors or parse errors) are logged and skipped. The extraction continues with remaining batches. If `WithIngestorLogger` is configured, failures are logged at `WARN` level.
