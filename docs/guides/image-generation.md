...
Use a Network to combine image generation with other capabilities. Attachments generated by sub-agents automatically propagate to the Network's `AgentResult.Attachments`:
...
result, err := network.Execute(ctx, oasis.AgentTask{
    Input: "Draw a sunset over the ocean",
})
// result.Attachments contains images from the artist sub-agent
```
