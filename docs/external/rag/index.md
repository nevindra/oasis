...
2. **Extract text.** The content type is inferred from the filename extension. The matching extractor runs: `PDFExtractor`, `DOCXExtractor`, `MarkdownExtractor`, `CSVExtractor`, `JSONExtractor`, `HTMLExtractor`, or `PlainTextExtractor`. Extractors that also implement `MetadataExtractor` return per-page metadata (page numbers, headings).
