...
// truncateStr truncates a string to n runes (not bytes).
...
// Suspend signals the workflow to pause execution.
...
// NodeType describes the type of a workflow node.
...
func (c *WorkflowContext) addUsage(u core.Usage) {
	c.mu.Lock()
	defer c.mu.Unlock()
	if existing, ok := c.values[usageKey]; ok {
		if eu, ok := existing.(core.Usage); ok {
...
const (
	...
	// StepSuspended indicates a step that was suspended awaiting external input.
	StepSuspended StepStatus = "suspended"
)

// resumeDataKey is the context key where resume data is stored.
