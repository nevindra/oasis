...
func (w *Workflow) runDAG(ctx context.Context, state *executionState, ch chan<- core.StreamEvent) {
...
func (w *Workflow) executeStep(ctx context.Context, s *stepConfig, state *executionState, ch chan<- core.StreamEvent) {
	...
	if ch != nil {
		select {
		case ch <- core.StreamEvent{Type: core.EventStepStart, Name: s.name}:
		case <-ctx.Done():
		}
	}
...
func (w *Workflow) recordStepOutcome(s *stepConfig, state *executionState, err error, stepSpan Span, duration time.Duration, endSpan func(string), ch chan<- core.StreamEvent) {
	...
	if err != nil {
		...
		if ch != nil {
			select {
			case ch <- core.StreamEvent{Type: core.EventStepFinish, Name: s.name, Content: err.Error(), Duration: duration}:
			default:
			}
		}
		if stepSpan != nil {
	...
	if ch != nil {
		select {
		case ch <- core.StreamEvent{Type: core.EventStepFinish, Name: s.name, Content: output, Duration: duration}:
		default:
		}
	}
	endSpan("success")
