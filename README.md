...
| **Tools** | `tools/data` (CSV/JSON transform), `tools/http` (typed fetch). Sandbox tools (`shell`, `execute_code`, `file_read`, `file_write`, `file_edit`, `file_search`, `browser`, `deliver_file`, MCP wrappers) are auto-registered via `agent.WithSandbox(sb, sandbox.Tools(sb)...)`. |
