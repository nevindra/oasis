...
// skill_* tools expose skill management to agents through atomic AnyTool
// implementations. Each operation (discover, activate, create, update) is
// its own AnyTool. Registered automatically when WithSkills is used.
...
func (t *skillDiscoverTool) Definition() ToolDefinition {
...
func (t *skillDiscoverTool) ExecuteRaw(ctx context.Context, _ json.RawMessage) (ToolResult, error) {
	summaries, err := t.provider.Discover(ctx)
	if err != nil {
		return ToolResult{Error: "discover failed: " + err.Error()}, nil
	}
	if len(summaries) == 0 {
...
func (t *skillActivateTool) Definition() ToolDefinition {
	return ToolDefinition{
		Name:        "skill_activate",
...
func (t *skillActivateTool) ExecuteRaw(ctx context.Context, args json.RawMessage) (ToolResult, error) {
	var p struct {
...
func (t *skillCreateTool) Definition() ToolDefinition {
	return ToolDefinition{
		Name:        "skill_create",
...
func (t *skillCreateTool) ExecuteRaw(ctx context.Context, args json.RawMessage) (ToolResult, error) {
	var p struct {
		Name         string   `json:"name"`
	...
	if p.Name == "" || p.Description == "" || p.Instructions == "" {
		return ToolResult{Error: "name, description, and instructions are required"}, nil
	}
	...
	return ToolResult{Content: fmt.Sprintf("created skill %q", sk.Name)}, nil
}
...
func (t *skillUpdateTool) Definition() ToolDefinition {
	return ToolDefinition{
		Name:        "skill_update",
...
func (t *skillUpdateTool) ExecuteRaw(ctx context.Context, args json.RawMessage) (ToolResult, error) {
	var p struct {
		Name         string   `json:"name"`
		Description  *string  `json:"description"`
		Instructions *string  `json:"instructions"`
	...
	if err != nil {
		return ToolResult{Error: "cannot update: " + err.Error()}, nil
	}
	...
	if len(changes) == 0 {
		return ToolResult{Content: "no changes specified"}, nil
	}
	if err := t.writer.UpdateSkill(ctx, p.Name, existing); err != nil {
		return ToolResult{Error: err.Error()}, nil
	}
	return ToolResult{Content: fmt.Sprintf("updated skill %q: %s", existing.Name, strings.Join(changes, ", "))}, nil
}

// Compile-time interface checks.
var (
	_ AnyTool = (*skillDiscoverTool)(nil)
	_ AnyTool = (*skillActivateTool)(nil)
	_ AnyTool = (*skillCreateTool)(nil)
	_ AnyTool = (*skillUpdateTool)(nil)
)
