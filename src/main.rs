...
async fn create_scheduler(
    config: &Config,
) -> Option<oasis_scheduler::scheduler::Scheduler> {
    ...
    let tasks = oasis_tasks::manager::TaskManager::new(task_db);
    ...
    let scheduler =
        oasis_scheduler::scheduler::Scheduler::new(db, tasks, bot, chat_id, config.brain.timezone_offset);
